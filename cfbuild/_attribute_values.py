...
from ._ncml_comments import (
    UNITS_WARNING,
    STANDARD_NAME_WARNING,
    TEMPORAL_UNITS_WARNING,
    OPENDAP_RESERVED_KEYWORDS_WARNING,
    MISSING_VALUES_ERROR,
    MONOTONIC_VALUES_ERROR,
    CANT_GET_VALUES_WARNING,
    MULTIDIMENSIONAL_WARNING,
    VALID_RANGE_WARNING,
    FILL_VALUE_WARNING,
    MISSING_VALUE_WARNING,
    VARIABLE_NAME_WARNING,
    BOUNDARY_VARIABLE_ATTRIBUTES_WARNING,
)
...
def _check_variable(variable, variable_values):
    ...
    if (
        not bool(re.match("^[A-Za-z0-9_]*$", variable.name))
        and variable.name[0].isalpha()
    ):
...
def _check_attribute_values(
    merged_attributes, variable, standard_name_table, variable_values
):
    if "standard_name" in merged_attributes:
        tree = etree.parse(standard_name_table)
        root = tree.getroot()
        standard_name_element = root.findall(
            'entry[@id="' + merged_attributes["standard_name"] + '"]'
        )
        ...
        if merged_attributes["standard_name"] == "latitude":
            ...
            if "units" in merged_attributes:
                if WARNING_MESSAGE in str(merged_attributes["units"]):
                    merged_attributes["units"] = {
                        "value": "!!!CHANGE ME!!! - (temporal units) since (date)",
        ...
        if len(standard_name_element) > 0:
            ...
            if len(standard_units) > 0:
                ...
                if "units" in merged_attributes:
                    if not isinstance(merged_attributes["units"], dict):
                        ...
                        if (
                            not standard_units.equivalent(units)
                            and "since" not in merged_attributes["units"]
                        ):
                            if WARNING_MESSAGE not in merged_attributes["units"]:
                                merged_attributes["units"] = {
                                    "value": str(units),
                                    "comment": UNITS_WARNING[0]
                                    + str(units)
                                    + UNITS_WARNING[1]
                                    + str(standard_units)
                                    + UNITS_WARNING[2],
            ...
            if WARNING_MESSAGE not in value:
                merged_attributes["standard_name"] = {
                    "value": value,
                    "comment": STANDARD_NAME_WARNING[0]
                    + value
    ...
    if "_FillValue" in merged_attributes:
        if (
            not isinstance(variable_values, numpy.ndarray)
            or len(variable_values.shape) <= 0
        ):
            del merged_attributes["_FillValue"]
        else:
            if isinstance(merged_attributes["_FillValue"], str):
                if (
                    not isinstance(variable_values, numpy.ndarray)
                    or len(variable_values.shape) <= 0
                ):
    ...
    if "missing_value" in merged_attributes:
        if (
            ...
            if isinstance(merged_attributes["missing_value"], str):
                if WARNING_MESSAGE in str(merged_attributes["missing_value"]):
                    if "_FillValue" in merged_attributes:
                        merged_attributes["missing_value"] = merged_attributes[
                            "_FillValue"
                        ]
                    else:
    ...
    if "valid_min" in merged_attributes or "valid_max" in merged_attributes:
        ...
        if (
            ...
            if isinstance(merged_attributes["valid_range"], str):
                if WARNING_MESSAGE in str(merged_attributes["valid_range"]):
                    ...
                    if data_type == "int":
                        ...
                        if fill_value is None:
                            ...
                            valid_min = (
                                fill_value + 2 * numpy.finfo(variable.data_type).eps
                            )
                            valid_max = numpy.finfo(variable.data_type).max
                        elif fill_value >= 0:
                            valid_min = numpy.finfo(variable.data_type).min
    ...
    if "actual_range" in merged_attributes:
        if (
            ...
            if isinstance(merged_attributes["actual_range"], str):
    ...
    if "valid_range" in merged_attributes and "actual_range" in merged_attributes:
        if not isinstance(merged_attributes["actual_range"], str) and not isinstance(
            merged_attributes["valid_range"], str
        ):
            if (
                merged_attributes["actual_range"][0]
                <= merged_attributes["valid_range"][0]
                or merged_attributes["actual_range"][1]
                >= merged_attributes["valid_range"][1]
            ):
                merged_attributes["valid_range"] = {
                    "value": merged_attributes["valid_range"],
                    "comment": VALID_RANGE_WARNING,
                }

    if "_FillValue" in merged_attributes and "actual_range" in merged_attributes:
        if not isinstance(merged_attributes["_FillValue"], str) and not isinstance(
            merged_attributes["valid_range"], str
        ):
            if (
                merged_attributes["valid_range"][0]
                <= merged_attributes["_FillValue"]
                <= merged_attributes["valid_range"][1]
            ):
                merged_attributes["_FillValue"] = {
                    "value": merged_attributes["_FillValue"],
                    "comment": FILL_VALUE_WARNING,
                }

    if "missing_value" in merged_attributes and "actual_range" in merged_attributes:
        if not isinstance(merged_attributes["missing_value"], str) and not isinstance(
            merged_attributes["valid_range"], str
        ):
            if (
                merged_attributes["valid_range"][0]
                <= merged_attributes["missing_value"]
                <= merged_attributes["valid_range"][1]
            ):
                merged_attributes["missing_value"] = {
                    "value": merged_attributes["missing_value"],
    ...
    if variable_values is not None:
        if len(variable_values.shape) == 1:
            ...
            if numpy.isnan(array_sum) or numpy.ma.is_masked(variable_values):
                warning_list.append(
                    f"{MISSING_VALUES_ERROR[0]}{variable.name}{MISSING_VALUES_ERROR[1]}"
                )
            if not numpy.all(
                variable_values[1:] > variable_values[:-1], axis=0
            ) and not numpy.all(variable_values[1:] < variable_values[:-1], axis=0):
                warning_list.append(
                    f"{MONOTONIC_VALUES_ERROR[0]}{variable.name}{MONOTONIC_VALUES_ERROR[1]}"
                )
        else:
            warning_list.append(f"{MULTIDIMENSIONAL_WARNING}")
    else:
        warning_list.append(
            f"{CANT_GET_VALUES_WARNING[0]}{variable.name}{CANT_GET_VALUES_WARNING[1]}"
        )
    return warning_list
        ...
        for variable_name in domain:
            ...
            if variable.variable_type == VARIABLE_TYPE_INDICATORS["Y"]:
                if "standard_name" in variable.attributes:
                    if (
                        variable.attributes["standard_name"] == "latitude"
                        or variable.attributes["standard_name"] == "grid_latitude"
                    ):
                        has_lat = True
                    elif (
                        variable.attributes["standard_name"]
                        == "projection_y_coordinate"
                        or variable.attributes["standard_name"]
                ...
                if "standard_name" in variable.attributes:
                    if (
                        variable.attributes["standard_name"] == "longitude"
                        or variable.attributes["standard_name"] == "grid_longitude"
                    ):
                        has_lon = True
                    elif (
                        variable.attributes["standard_name"]
...
def _fill_global_attributes(required_attributes, dataset):
    ...
    if "date_metadata_modified" in required_attributes:
        if WARNING_MESSAGE in str(required_attributes["date_metadata_modified"]):
            required_attributes["date_metadata_modified"] = (
    ...
    if "history" in required_attributes:
        ...
        if WARNING_MESSAGE in str(required_attributes["history"]):
            required_attributes["history"] = (
                f"{date}, Metadata updated using cfbuild python package"
            )
        else:
            required_attributes[
                "history"
    ...
    if "geospatial_bounds_crs" in required_attributes:
        if dataset.crs is not None and dataset.crs != "":
            ...
            for variable in dataset.variables:
                if (
                    dataset.variables[variable].variable_type
        ...
        if WARNING_MESSAGE in str(required_attributes["geospatial_bounds"]):
            ...
            if x_dim is not None and y_dim is not None:
                try:
                    ...
                    wkt_string = (
                        f"POLYGON (({y_min} {x_min}, {y_max} {x_min}, {y_max} {x_max}, {y_min} {x_max}, "
                        f"{y_min} {x_min}))"
                    )
                    ...
                    if "geospatial_lat_resolution" in required_attributes:
                        if WARNING_MESSAGE in str(
                            required_attributes["geospatial_lat_resolution"]
                        ):
                            required_attributes["geospatial_lat_resolution"] = (
                                y_resolution
                            )
                    if "units" in x_dim.attributes:
                        if "geospatial_lon_units" in required_attributes:
                            if WARNING_MESSAGE in str(
                                required_attributes["geospatial_lon_units"]
                            ):
                                required_attributes["geospatial_lon_units"] = (
                                    x_dim.attributes["units"]
                                )
                    if "units" in y_dim.attributes:
                        if "geospatial_lat_units" in required_attributes:
                            if WARNING_MESSAGE in str(
                                required_attributes["geospatial_lat_units"]
                            ):
                                required_attributes["geospatial_lat_units"] = (
                                    y_dim.attributes["units"]
                                )
    ...
    if t_dim is not None:
        try:
            ...
            if t_units is not None:
                ...
                t_difference_date = netCDF4.num2date(
                    t_difference_num, t_units, t_calendar
                )
                ...
                t_resolution_string = isodate.duration_isoformat(
                    t_difference_date - t_min_date
                )

                if "time_coverage_start" in required_attributes:
                    if WARNING_MESSAGE in str(
                        required_attributes["time_coverage_start"]
                    ):
                ...
                if "time_coverage_duration" in required_attributes:
                    if WARNING_MESSAGE in str(
                        required_attributes["time_coverage_duration"]
                    ):
                        required_attributes["time_coverage_duration"] = (
                            t_duration_string
                        )
                ...
                if "time_coverage_resolution" in required_attributes:
                    if WARNING_MESSAGE in str(
                        required_attributes["time_coverage_resolution"]
                    ):
                        required_attributes["time_coverage_resolution"] = (
                            t_resolution_string
                        )
...
def _determine_global_attributes_for_given_conventions(
    conventions, current_conventions, group
):
    append_convention = True
    if (
        CONVENTION_VERSIONS[0] in conventions
        and not CONVENTION_VERSIONS[1] in conventions
    ):
        ...
        for convention in current_conventions:
            ...
            if "CF" in convention:
                ...
                current_conventions = (
                    current_conventions[:list_index]
                    + [CONVENTION_VERSIONS[0]]
        ...
        CONVENTION_VERSIONS[1] in conventions
        and not CONVENTION_VERSIONS[0] in conventions
    ):
        GLOBAL_ATTRIBUTES["ACDD_ATTRIBUTES"]["date_created"] = datetime.now().strftime(
            "%m/%d/%Y"
        )
        ...
        required_attributes = {
            **GLOBAL_ATTRIBUTES["CF_GLOBAL_ATTRIBUTES"],
            **GLOBAL_ATTRIBUTES["ACDD_ATTRIBUTES"],
        }
