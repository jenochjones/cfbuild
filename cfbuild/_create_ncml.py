...
def create_ncml(ds):
    def add_attribute(element, name, value, last_attribute, indent_level: int = 1):
        attribute_element = etree.SubElement(
            element, "attribute", name=str(name), value=str(value)
        )
        if last_attribute:
    ...
    def add_group_attributes(group, group_element, required_attributes, indent_level):
        merged_attributes = sort_and_merge_attribute_lists(
            required_attributes, group.attributes
        )
    ...
    def _determine_variable_attributes(variable, variable_element, indent_level):
        ...
        if (
            variable_type == VARIABLE_TYPE_INDICATORS["C"]
            or variable_type == VARIABLE_TYPE_INDICATORS["T"]
            or variable_type == VARIABLE_TYPE_INDICATORS["X"]
            or variable_type == VARIABLE_TYPE_INDICATORS["Y"]
            or variable_type == VARIABLE_TYPE_INDICATORS["Z"]
        ):
            required_attributes = sort_and_merge_attribute_lists(
                required_attributes,
                CF_DATA_TYPE_ATTRIBUTES[VARIABLE_TYPE_INDICATORS["C"]],
            )

        required_attributes = sort_and_merge_attribute_lists(
            required_attributes, CF_DATA_TYPE_ATTRIBUTES[variable_type]
        )
        ...
        merged_attributes = sort_and_merge_attribute_lists(
            required_attributes, current_variable_attributes
        )
        ...
        merged_attributes = _check_attribute_values(
            merged_attributes, variable, ds.standard_name_table, variable_values
        )
    ...
    def iterate_group(
        ...
        conventions: list = CONVENTION_VERSIONS,
        indent_level: int = 0,
    ):
        if top_level:
            ...
            required_attributes = _determine_global_attributes_for_given_conventions(
        ...
        for index, dimension in enumerate(group.dimensions):
            dimension_element = etree.SubElement(
                group_element,
                "dimension",
                name=group.dimensions[dimension].name,
            ...
            if index + 1 < len(group.dimensions):
                ...
                dimension_element.tail = "\n\n" + "\t" * indent_level
                xml_comment = etree.Comment(
                    VARIABLE_COMMENTS[VARIABLE_TYPE_INDICATORS["T"]]
                )
                xml_comment.tail = "\n\t"
                group_element.append(xml_comment)
        ...
        for index, variable in enumerate(variable_ordered_dictionary):
            ...
            if str(variable.data_type) not in DATA_TYPES:
                xml_comment = etree.Comment(
                    f"{DATA_TYPE_WARNING[0]}{variable.data_type}{DATA_TYPE_WARNING[1]}"
                )
                xml_comment.tail = "\n" + "\t" * (indent_level + 1)
                group_element.append(xml_comment)
            ...
            if index + 1 < len(variable_ordered_dictionary):
                if (
                    variable_ordered_dictionary[index + 1].variable_type
                    != variable.variable_type
                ):
                    variable_element.tail = "\n\n" + "\t" * indent_level
                    variable_comment = VARIABLE_COMMENTS[
                        variable_ordered_dictionary[index + 1].variable_type
                    ]
        ...
        if len(group.groups) > 0:
            for group in ds.groups:
                ...
                iterate_group(
                    group, group_element, False, conventions, indent_level + 1
                )
