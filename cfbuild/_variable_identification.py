...
def determine_variable_type(netCDF4_dataset):
    ...
    for variable in variables:
        if (
            len(variables[variable].dimensions) == 1
            and variables[variable].dimensions[0] == variable
        ):
            if hasattr(variables[variable], "axis"):
                if variables[variable].axis == "X":
                    if (
                        VARIABLE_TYPE_INDICATORS["X"]
                        not in variable_type_dict[variable]
                    ):
                        variable_type_dict[variable].append(
                            VARIABLE_TYPE_INDICATORS["X"]
                        )
                ...
                if variables[variable].standard_name in STANDARD_NAMES:
                    if (
                        STANDARD_NAMES[variables[variable].standard_name]
                        not in variable_type_dict[variable]
                    ):
                        variable_type_dict[variable].append(
                            STANDARD_NAMES[variables[variable].standard_name]
            ...
            if (
                VARIABLE_TYPE_INDICATORS["X"] not in variable_type_dict[variable]
                and VARIABLE_TYPE_INDICATORS["Y"] not in variable_type_dict[variable]
                and VARIABLE_TYPE_INDICATORS["Z"] not in variable_type_dict[variable]
                and VARIABLE_TYPE_INDICATORS["T"] not in variable_type_dict[variable]
            ):
        ...
        if hasattr(variables[variable], "grid_mapping"):
            ...
            if grid_mapping_variable in variable_type_dict:
                if (
                    ...
                    not in variable_type_dict[grid_mapping_variable]
                ):
                    variable_type_dict[grid_mapping_variable].append(
                        VARIABLE_TYPE_INDICATORS["G"]
                    )
        ...
        if hasattr(variables[variable], "bounds"):
            ...
            if boundary_variable in variable_type_dict:
                if (
                    ...
                    not in variable_type_dict[boundary_variable]
                ):
                    variable_type_dict[boundary_variable].append(
                        VARIABLE_TYPE_INDICATORS["B"]
                    )
                ...
                if ancillary_variable in variable_type_dict:
                    if ancillary_variable in variable_type_dict:
                        if (
                            ...
                            not in variable_type_dict[ancillary_variable]
                        ):
                            variable_type_dict[ancillary_variable].append(
                                VARIABLE_TYPE_INDICATORS["AD"]
                            )
                ...
                if auxillary_variable in variable_type_dict:
                    if len(variables[auxillary_variable].dimensions) == 0:
                        if (
                            ...
                            not in variable_type_dict[auxillary_variable]
                        ):
                            variable_type_dict[auxillary_variable].append(
                                VARIABLE_TYPE_INDICATORS["S"]
                            )
                    elif auxillary_variable in variable_type_dict:
                        if (
                            auxillary_variable
                            not in variables[auxillary_variable].dimensions
                        ):
                            if (
        ...
        if hasattr(variables[variable], "cell_measures"):
            ...
            for character in cell_measures_string:
                if character == ":" and cell_measures_string[counter] == " ":
                    cell_measures_string = (
                        cell_measures_string[:counter]
                        + cell_measures_string[counter + 1 :]
                    )
                else:
                    counter += 1
            ...
            for cell_measures_variable in cell_measures_variable_list:
                if cell_measures_variable in variable_type_dict:
                    if (
                        ...
                        not in variable_type_dict[cell_measures_variable]
                    ):
                        variable_type_dict[cell_measures_variable].append(
                            VARIABLE_TYPE_INDICATORS["CM"]
                        )
