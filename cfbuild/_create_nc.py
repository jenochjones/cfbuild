...
# from ._constants import VARIABLE_TYPE_INDICATORS # F401: Unused
...
def create_or_update_nc_file(ncml_object, final_netcdf4_dataset, original_dataset):
    ...
    def parse_attribute_value(element, number_type):
        if validate_json(element.attrib["value"]):
            if element.attrib["value"][0] == "[" and element.attrib["value"][-1] == "]":
                ...
                if number_type is not None and (
                ...
                if element.tag.split("}")[-1] == "attribute":
                    if element.attrib["name"] not in netcdf_element.ncattrs():
                        if element.attrib["name"] == "units":
                            attribute_dictionary[element.attrib["name"]] = str(
    ...
    def iterate_group(
        xml_element,
        final_netcdf4_dataset_group,
        cfbuild_dataset_group,
        ...
        for element in xml_element:
            ...
            if element.tag is not etree.Comment:
                if element.tag.split("}")[-1] == "dimension":
                    if (
                        element.attrib["length"] == "None"
                        or element.attrib["length"] == "unlimited"
                    ):
                        dimension_size = None
                    ...
                    if (
                        element.attrib["name"]
                        not in final_netcdf4_dataset_group.dimensions
                    ):
                        final_netcdf4_dataset_group.createDimension(
                            dimname=element.attrib["name"], size=dimension_size
                        )

                elif element.tag.split("}")[-1] == "variable":
                    shape_str = element.attrib["shape"]
                    try:
                        # Safely evaluate the shape string, e.g., "('time', 'lat', 'lon')"
                        parsed_shape = ast.literal_eval(shape_str)
                        if not isinstance(parsed_shape, tuple):
                            raise ValueError("Shape attribute must be a tuple string.")
                    except (ValueError, SyntaxError) as e:
                        # Handle error: log, raise, or use a default
                        print(f"Error parsing shape attribute '{shape_str}': {e}")
                        # Potentially raise a more specific error or skip this variable
                        continue

                    has_values, element_values = check_element_for_values(
                        element, parsed_shape, final_netcdf4_dataset_group.dimensions
                    )
                    ...
                    if has_values:
                        ...
                        try:
                            variable_values = original_dataset_group.variables[
                                original_name
                            ][:]
                    ...
                    if new_name in final_netcdf4_dataset_group.variables:
                        ...
                        if isinstance(variable_values, numpy.ndarray):
                            if "_FillValue" in attributes:
                                if (
                                    isinstance(attributes["_FillValue"], str)
                                    or isinstance(attributes["_FillValue"], bytes)
                                    or isinstance(attributes["_FillValue"], bytearray)
                                ):
                                    if validate_json(attributes["_FillValue"]):
                                        fill_value = json.loads(
                            ...
                            add_attributes(
                                element, new_variable, element.attrib["type"]
                            )

                            if element.attrib["type"] != str(variable_values.dtype):
                                new_variable[:] = variable_values.astype(
                                    str(element.attrib["type"])
                                )
                            else:
                                new_variable[:] = variable_values

                elif element.tag.split("}")[-1] == "group":
                    new_group = final_netcdf4_dataset_group.createGroup(
                        groupname=element.attrib["name"]
                    )
                    iterate_group(
                        element,
                        new_group,
                        original_dataset_group.groups[element.attrib["name"]],
                    )
